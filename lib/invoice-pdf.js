...
import { PAYOUT_EUR_PER_STAR } from './config.js';
...
export async function generateInvoicePdf({ payout, profile, purchases, invoiceNumber }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const eurPerStar = Number(payout.eur_per_star || PAYOUT_EUR_PER_STAR);
      const totalStars = Number(payout.amount_stars || 0);
      const grossEur = payout.amount_eur_cents != null
        ? Number((Number(payout.amount_eur_cents) / 100).toFixed(2))
        : Number((totalStars * eurPerStar).toFixed(2));
      ...
      // ── Header ──
      doc.fontSize(20).font('Helvetica-Bold').text('PAYOUT STATEMENT', 50, 50);
      ...
      doc.text(
        'The platform fee (capped at 5%) is already deducted from the creator share. Transfer fees shown above are charged by the payout provider.',

...
export async function generateInvoicePdf({ payout, profile, purchases, invoiceNumber }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // ── Totals ──
      rowY += 8;
      doc.moveTo(300, rowY).lineTo(510, rowY).strokeColor('#e0e0e0').lineWidth(0.5).stroke();
      rowY += 8;

      doc.font('Helvetica').fillColor('#000000').fontSize(10);
      doc.text('Subtotal', 300, rowY, { width: 120 });
      doc.text(`${grossEur.toFixed(2)} EUR`, 430, rowY, { width: 80, align: 'right' });
      rowY += 18;

      doc.text(`Stars total: ${totalStars}`, 300, rowY, { width: 120 });
      doc.fillColor('#999999').text(`@ ${eurPerStar} EUR/Star`, 430, rowY, { width: 80, align: 'right' });
      rowY += 18;

      if (feeEur > 0) {
        doc.fillColor('#c62828');
        doc.text(`Transfer fee (${fee.description})`, 300, rowY, { width: 120 });
        doc.text(`-${feeEur.toFixed(2)} EUR`, 430, rowY, { width: 80, align: 'right' });
        rowY += 18;
      }

      doc.moveTo(300, rowY).lineTo(510, rowY).strokeColor('#7c3aed').lineWidth(1).stroke();
      rowY += 10;

      doc.font('Helvetica-Bold').fillColor('#7c3aed').fontSize(14);
      doc.text('TOTAL DUE', 300, rowY, { width: 120 });
      doc.text(`${netEur.toFixed(2)} EUR`, 430, rowY, { width: 80, align: 'right' });

...
export function starsToUsd(stars) {
  return Number((Number(stars) * USD_PER_STAR).toFixed(2));
}

export function formatStarsWithEur(stars) {
  const s = Number(stars);
  if (!s) return '0 Stars';
  const eur = starsToEur(s);
  return `${s} Stars (~${eur.toFixed(2)} EUR)`;
}

...
const parsedInitDataMaxAge = Number.parseInt(process.env.INIT_DATA_MAX_AGE_SECONDS || '86400', 10);
// Mini App sessions can stay open for a while; too-low TTL breaks publish flow unexpectedly.
// Enforce a sane minimum (24h) unless explicitly configured higher.
export const INIT_DATA_MAX_AGE_SECONDS = Number.isFinite(parsedInitDataMaxAge) && parsedInitDataMaxAge > 0
  ? Math.max(parsedInitDataMaxAge, 86400)
  : 86400;

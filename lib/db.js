...
export async function createProduct(
  ...
  const db = getClient();
  await db.execute({
    sql: `INSERT INTO products (id, creator_id, title, description, price_stars, content_type, content, file_id, price_usd_cents, price_eur_cents, payment_methods)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [id, creatorId, title, description, priceStars, contentType, content, fileId, priceUsdCents, priceEurCents, paymentMethods],
  });
...
export async function getPayoutDetails(payoutId) {
  ...
  const payout = await db.execute({
    sql: 'SELECT id, creator_id, amount_stars, status, created_at, paid_at FROM payouts WHERE id = ? LIMIT 1',
    args: [Number(payoutId)],
  });

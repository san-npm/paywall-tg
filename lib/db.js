...
async function _ensureTablesImpl() {
  ...
  const migrations = [
    ...
    // Backfill consumed ids from existing card purchases so the replay guard
    // also covers rows created before this ledger existed. Idempotent (OR IGNORE
    // against the unique indexes); a no-op once every row is present.
    "INSERT OR IGNORE INTO stripe_fulfillments (stripe_session_id, stripe_payment_intent_id, product_id, buyer_telegram_id) SELECT stripe_session_id, stripe_payment_intent_id, product_id, buyer_telegram_id FROM fiat_purchases",
  ];
...
/**
 * Reactivate a previously-refunded purchase for the same (product, buyer).
 * The UNIQUE(product_id, buyer_telegram_id) constraint blocks a fresh INSERT when
 * a refunded row already exists, so a legitimate re-purchase after a refund must
 * update that row in place. Returns true if a refunded row was reactivated.
 */
export async function reactivatePurchase(productId, buyerTelegramId, starsPaid, creatorShare, platformFee, chargeId) {
  await ensureTables();
  const db = getClient();
  const result = await db.execute({
    sql: `UPDATE purchases
      SET refunded = 0, telegram_charge_id = ?, stars_paid = ?, creator_share = ?, platform_fee = ?,
          payout_id = NULL, created_at = datetime('now')
      WHERE product_id = ? AND buyer_telegram_id = ? AND refunded = 1`,
    args: [chargeId, starsPaid, creatorShare, platformFee, productId, buyerTelegramId],
  });
...
export async function recordFiatPurchase(
  ...
  await db.execute({ sql: 'UPDATE products SET sales_count = sales_count + 1 WHERE id = ?', args: [productId] });
  await recordStripeFulfillment(stripeSessionId, stripePaymentIntentId, productId, buyerTelegramId);
}

/**
 * Reactivate a previously-refunded fiat (Stripe) purchase for the same (product, buyer).
 * Mirrors reactivatePurchase for the Stars path: UNIQUE(product_id, buyer_telegram_id)
 * blocks a fresh INSERT while a refunded row still exists, so a legitimate re-purchase
 * after a refund must update that row in place. Returns true if a refunded row was reactivated.
 */
...
/**
 * Append the consumed Stripe ids to the immutable fulfillment ledger. Called
 * whenever card access is granted (record or reactivate). Best-effort and
 * idempotent: a duplicate id is a no-op (the UNIQUE indexes reject it), and a
 * transient failure leaves us no worse than before the ledger existed.
 */
export async function recordStripeFulfillment(stripeSessionId, stripePaymentIntentId, productId, buyerTelegramId) {
  const sid = stripeSessionId ? String(stripeSessionId) : null;
  const pi = stripePaymentIntentId ? String(stripePaymentIntentId) : null;
  if (!sid && !pi) return;
  try {
    await getClient().execute({
      sql: `INSERT INTO stripe_fulfillments (stripe_session_id, stripe_payment_intent_id, product_id, buyer_telegram_id)
        VALUES (?, ?, ?, ?)`,
      args: [sid, pi, productId, buyerTelegramId],
    });
  } catch (err) {
    // This is a redundant replay guard, never a source of truth, and it runs
    // AFTER the purchase row is already committed. It must never abort a
    // committed fulfillment, so swallow ALL errors: a duplicate id is expected,
    // and a transient error just means this guard row is missing (no worse than
    // before the ledger existed). Never rethrow.
    if (!isUniqueConstraintError(err)) {
      console.error('recordStripeFulfillment non-fatal error:', err?.message || err);
    }
  }
}
  ...
  for (const row of creatorsRes.rows) {
    ...
    try {
      ...
      if (assigned.rowsAffected === 0) {
        await db.execute({ sql: 'DELETE FROM payouts WHERE id = ?', args: [payoutId] });
        await db.execute('COMMIT');
        continue;
      }
      // Update payout amount to reflect actual assigned purchases (may differ from pre-read)
      const actualSum = await db.execute({
        sql: 'SELECT COALESCE(SUM(creator_share),0) as total FROM purchases WHERE payout_id = ?',
        args: [payoutId],
      });
      const actualAmount = Number(actualSum.rows[0]?.total || 0);
      if (actualAmount !== Number(row.amount_stars)) {
        await db.execute({ sql: 'UPDATE payouts SET amount_stars = ? WHERE id = ?', args: [actualAmount, payoutId] });
      }
      await db.execute('COMMIT');

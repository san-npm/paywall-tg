...
import { VALID_PAYOUT_METHODS } from './constants.js';
...
export function validateInitDataDetailed(initData) {
  ...
  params.delete('hash');
  // Modern Telegram clients also send an Ed25519 `signature` field (third-party
  // validation). Telegram excludes BOTH hash and signature from the bot-token
  // data_check_string, so signature must be stripped too or the HMAC will not
  // match and legitimate initData is rejected. Deleting an absent field is a no-op.
...
export function generateShortId() {
  ...
  // Rejection sampling to avoid modulo bias (256 % 30 != 0)
 ...
 */
export function sanitizeErrorMessage(msg, fallback = 'An unexpected error occurred') {
  if (!msg || typeof msg !== 'string') return fallback;
  // Length-cap BEFORE the regex so a pathological input can't drive backtracking.
  if (msg.length > 200) return fallback;
  if (/\b(SQLITE|SQL|syntax error|stack|at\s+\w+\s+\(|\/home\/|node_modules)/i.test(msg)) return fallback;
  return msg;
}
